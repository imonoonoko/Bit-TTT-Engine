...
impl DataLoader {
    ...
    fn next_batch(
        ...
        // OPTIMIZATION: Use pre-allocated vectors if possible, but here we just ensure capacity.
        // Ideally these should be members of DataLoader to reuse memory across batches.
...
struct Args {
    ...
    /// Run in benchmark mode (no saving, pure speed test)
...
fn main() -> Result<()> {
    ...
    if args.benchmark {
        ...
        println!("   - Checkpoints: DISABLED");
        println!("   - State Saving: DISABLED");
        println!("   - Target: {} steps", args.steps);
    } else {
        // --- Auto-Save Config (Normal Mode) ---
    ...
    let start_time = std::time::Instant::now(); // For overall speed tracking
        ...
        */
        ...
        if step % log_interval == 0 {
            ...
            let avg_tokens_per_sec = if elapsed > 0.0 {
                ((step - start_step + 1) as f64 / elapsed) * BATCH_SIZE as f64 * CONTEXT_LEN as f64
            } else {
                0.0
            };
