...
// use memmap2::Mmap; // Moved to loader.rs
use std::fs::File;
...
pub fn run(args: TrainArgs) -> Result<()> {
    ...
    let project_config = crate::config::ProjectConfig {
        ...
        n_heads: 8, // Hardcoded in args default usually, but we set it here
    ...
    let config = project_config.to_bit_llama_config(0.1); // inner_lr 0.1

    let mut varmap = VarMap::new();
    let vb = VarBuilder::from_varmap(&varmap, DType::F32, &device);
    let model = BitLlama::load(config.clone(), vb)?; // clone config if needed
    ...
    let checkpoint_path = if let Some(path) = &args.load {
        info!("📂 Loading specific checkpoint from Launcher: {}", path);
        Some(path.clone())
    ...
    for step in start_step..args.steps {
        ...
        if step % log_interval == 0 {
            ...
            // std::io::stdout().flush().ok(); // Tracing handles flushing usually
